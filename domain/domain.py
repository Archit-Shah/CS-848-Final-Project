...
class DomainEngine:
    ...
    def generate_domain(self):
        ...
        ii=0
        # np.random.seed(1000)
        for i,row in enumerate(tqdm(list(records))):
            tid = row['_tid_']
            for attr in self.active_attributes:
                # random_sampler = np.random.binomial(1,0.7)
                # if random_sampler == 0:
                if records[i][-1] == True and attr in Green:
                    samp = 0
                else:
                    samp = 1
                ii+=1
        ...
        print("Number of cells considered",ii)
        print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
        print("Describe initial DataFrame",domain_df.describe())
        ...
        print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
        print("Describe fina; DataFrame",domain_df.describe())
